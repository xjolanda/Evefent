...
public class DatabaseServices extends Service {
    ...
    public void enumerateEventProfiles() {
        if (debug)
        {
            ...
            sendUpdatedEvents(generateDummyEvents());
            return;
        }
    ...
    public void updateEventProfile(final int event_id) {
        queryEventProfile(event_id, new DatabaseServices.OnJSONResponseCallback() {
            ...
            public void onJSONResponse(boolean success, Context context, JSONObject response) {
               if(debug)
               {
                   ...
                   sendUpdatedEvent(generateDummyEvent(event_id));
                   return;
               }
                try
                {
    ...
    private void sendUpdatedEvent(EventProfile updatedEvent)
    {
        Intent intent = new Intent(getApplicationContext().getResources().getString(R.string.EventUpdatedMessage));
        intent.putExtra(getApplicationContext().getResources().getString(R.string.EventUpdatedMessage), updatedEvent);
